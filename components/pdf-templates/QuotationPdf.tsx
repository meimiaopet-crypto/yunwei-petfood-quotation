    ...
    <Document>
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.summaryBox}>
          ...
          <View style={styles.summaryRow}>
            <Text style={{ fontSize: 9, color: '#64748B' }}>{t.freight[lang]}{meta.logisticsType ? ` (${(tLogistics as any)[meta.logisticsType]?.[lang] ?? meta.logisticsType})` : ''}</Text>
              ...
              <View>
                ...
                {terms.map((term) => (
                  <Text key={term.id} style={{ fontSize: 8, marginBottom: 2 }}>
                    <Text style={{ fontWeight: 700 }}>· {term.name}：{termContent(term)}</Text>
                  </Text>
                ))}
              </View>
            )}
